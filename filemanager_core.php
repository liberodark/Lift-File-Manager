...
//        $txt = $this->quote($txt);
    ...
    public function adminInfo()
    {
        if (isset($_SESSION["filemanager_admin"])) {
            ...
            $query = "SELECT * FROM filemanager_db WHERE is_login='1' AND ck_id='$ck_id'";
    ...
    public function get_users()
    {
        ...
        if ($select) {
            ...
            while ($row = $select->fetchAll()) {
                $users['id'][] = $row["0"]["id"];
                $users['firstname'][] = $this->decode_me($row["0"]["firstname"]);
                $users['lastname'][] = $this->decode_me($row["0"]["lastname"]);
                $users['username'][] = $this->decode_me($row["0"]["username"]);
                $users['email'][] = $this->decode_me($row["0"]["email"]);
                $users['is_block'][] = $row["0"]["is_block"];
                $users['dir_path'][] = $this->decode_me($row["0"]["dir_path"]);
                $users['date_added'][] = $row["0"]["date_added"];
                $users['permissions'][] = $this->switch_user_permission($option->get_option("permission_for_" . $row["0"]["id"]));
                $users['filemanager_ext'][] = $option->get_option("allow_extensions_" . $row["0"]["id"]);
                $users['uploader_ext'][] = $option->get_option("allow_uploads_" . $row["0"]["id"]);
                $users['deny_folders'][] = $option->get_option("deny_folders_" . $row["0"]["id"]);
                $limitation = $option->get_option("user_limit_" . $row["0"]["id"]);
                $limitation = ($limitation * 1024) * 1024;
                $users["limitation"][] = $this->set_limitation($this->decode_me($row["0"]["dir_path"]), $limitation);
                $users["upload_limitation"][] = $option->get_option("user_upload_limit_" . $row["0"]["id"]);
            }
        }
        return $users;
    }
...
class filemanager extends Services_JSON
{
    ...
    public function get_support_ext()
    {
        ...
        $content = [];
        $query = $db->prepare("SELECT * FROM filemanager_options WHERE option_name='allow_extensions'");
        $query->execute();
...
class fs extends filemanager
{
    ...
    public function lst($id, $with_root = false)
    {
        ...
        $res = [];
        foreach ($lst as $item) {
            if ($item == '.' || $item == '..' || $item === null || in_array($item, $this->ignored)) {
                continue;
            }
            ...
            if (is_dir($dir . DIRECTORY_SEPARATOR . $item)) {
                ...
                if (!in_array(strtolower($ext), $this->support_ext)) {
                    continue;
                }
