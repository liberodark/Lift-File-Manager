...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) AND strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest')
{
    if ($core->isLogin())
    {
        ...
        {
            $core->adminInfo();
            if ($_POST["showTicket"] == $core->admin_id)
            {
                ...
                $tickets_class->admin_role_info = $core->admin_firstname." ".$core->admin_lastname;
                $tickets_class->admin_gravatar = $tickets_class->get_gravatar_src($core->admin_email);
...
<div class="modal fade" id="ticket_reply" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal"><?php language_filter("Close");?></button>
                <button type="button" class="btn btn-primary" onclick="reply_message('<?php echo $ticketId;?>', '<?php echo $core->admin_id;?>');"><?php language_filter("Send_Message");?></button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    function reply_message(id, adminId)
    {
        ...
        setTimeout(function(){
            ...
            "ajax_manage_tickets.php",
            {
                method:"reply_ticket",
                ticketId:id,
                userId:'<?php echo $tickets_class->ticket_user_id;?>',
                adminId:adminId,
                role:"admin",
                message:message
            },
        ...
        else
        {
            ...
            setTimeout(function(){
                ...
                "ajax_manage_tickets.php",
                {
                    ...
                    ticketId:id,
                    newStatus:method,
                    adminId:"<?php echo $core->admin_id;?>",
                    message:message
                },
                function (data, status)
                {
