...
class send_notifications extends filemanager_user_core
{
    public function send_mails($info)
    {
        ...
        $admin_link = str_replace("filemanager_user/upload.php", "", $this_file_path);
        ...
        {
            $select_admins = mysql_query("SELECT id, firstname, lastname, email FROM filemanager_db  LIMIT 1");
            if($select_admins)
            {
                while($row = mysql_fetch_array($select_admins))
                {
    ...
    private function send_admin_email($base_info, $admin_info)
    {
        ...
        $check_root = end(explode("/", trim(ROOT_DIR_PATH, "/")));
        if($folderName == $check_root)
        {
