    ...
    if ($core->isLogin())
    {
        if (isset($_POST["showAddUser"]))
        {
            ...
            if(isset($_POST["add_username"]) and isset($_POST["add_email"]))
            {
                ...
                $extra_dirs = $_POST["add_user_extra_dir"];
                $user_perm = array();
                ...
                $add = $core->add_new_user($username, $email, $firstname, $lastname, $password, $send_pass, $user_dir, $limitation, $upload_limitation, $deny_files, $extra_dirs, $user_perm, $user_ext, $user_up);
                if($add == true)
                {
...
?>
            ...
            <div class="modal fade" id="siteMap" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            <button type="button" id="select_dir_btn" class="btn btn-primary" data-dismiss="modal" onclick=""><?php language_filter("Select")?></button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <script>
                var userPermission = new Array(); // send
                var userExtensions = new Array(); // send
                var userUploader = new Array(); // send
                var lastSelectedFolder = "";
                var userExtraDirs = new Array();
                var lastSelectedDeny = "";
                var userDenyFiles = new Array();
                var selected_perms = new Array();
                ...
                /*"edit_img",*/
                var all_uploader = new Array(<?php for($i = 0; $i < count($allow_ext_uploader); $i++){if($i == (count($allow_ext_uploader) - 1)) echo "'".$allow_ext_uploader[$i]."'"; else echo "'".$allow_ext_uploader[$i]."', "; }?>);
                var all_filemanager = new Array(<?php for($i = 0; $i < count($allow_ext_filemanager); $i++){if($i == (count($allow_ext_filemanager) - 1)) echo "'".$allow_ext_filemanager[$i]."'"; else echo "'".$allow_ext_filemanager[$i]."', "; }?>);
                            ...
                            if(status == "success")
                            {
                                if(data == "done")
                                {
                                    $.post("ajax_add_user.php",
                                    {
                                        ...
                                        add_user_extra_dir:user_extra_dir,
