...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) AND strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest')
{
    if ($core->isLogin())
    {
        ...
        {
            $core->adminInfo();
            if ($_POST["showTickets"] == $core->admin_id)
            {
                $show = $_POST["show_what"];
                $page = $_POST["ticket_page"];
                $settings = $option->get_option("settings");
                if($show == "new") $tickets = "";
                else $tickets = $tickets_class->get_tickets($show);
?>
            <div class="row">
                <?php
                                ...
                                <form>
                                    <div class="new-ticket">
                                        <div class="col-md-6">
                                            ...
                                            <div class="form-group">
                                                <label for="ticket_user"><?php language_filter("User");?></label>
                                                <select id="ticket_user" name="ticket_user" class="form-control" required="required">
                                                    <option value="na" selected="selected"><?php language_filter("Choose_One");?></option>
                                                    <?php
                                                    if($users != "")
                                                    {
                                                        for($i = 0; $i < count($users["id"]); $i++)
                                                        {
                                                            echo '<option value="'.$users["id"][$i].'">'.$users["name"][$i].'</option>';
                                                        }
                                                    }
                                                    ?>
                                                </select>
                    ...
                    setTimeout(function(){
                        $.post(
                            'ajax_manage_tickets.php',
                            {
                                ...
                                message:message,
                                user:user,
                                method:"admin_create_ticket_for_user"
                            },
                            function (data, status)
                            {
                                if(status == "success")
