        ...
        if(isset($_GET["info"]))
        {
            ...
            if($core->check_base_root($file))
            {
                if(is_file($file))
                {
                    ...
                    <body>
                    <form method="post" style="width: 100%" action="edit_file.php?info=<?php echo base64_encode(utf8_encode($file));?>&switch=<?php echo @$_GET["switch"];?>">
                        <textarea style="width: 97%; height: 300px; margin: 10px 12px 0 12px;" class="form-control" name="new_value"><?php echo $input;?></textarea>
                        <hr style="width: 100%"/>
