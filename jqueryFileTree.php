...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) AND strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest')
{
    if ($core->isLogin())
    {
        ...
        $ext_allow = $core->core_get_support_ext();
        $_POST['dir'] = urldecode( $_POST['dir'] );
        $sign = "..";
        if(isset($_POST['sign']))
        {
            $sign = urldecode($_POST['sign']);
            if($sign == ROOT_DIR_PATH)
            {
                $sign = "..";
            }
            else
        ...
        if( file_exists(@$root . $DIR) ) {
            ...
            if( count($files) > 2 ) { /* The 2 accounts for . and .. */
                ...
                // All dirs
                if($_POST["dir"] == ROOT_DIR_PATH and !isset( $_GET["showFile"] ) )
                    echo "<li class=\"directory \"><a href=\"javascript:;\" id=\"filemanager_home_directory_root\" rel=\"".$dir_root."\">" . htmlentities($dir_root) . "</a></li>";

                foreach( $files as $file ) {
                    if (in_array($file, $ignored)) continue;
                    if( file_exists(@$root . $DIR . $file) && $file != '.' && $file != '..' && is_dir(@$root . $DIR . $file) ) {
                        echo "<li class=\"directory collapsed\"><a href=\"#\" rel=\"" . htmlentities($DIR . $file, ENT_QUOTES, "UTF-8") . "/\">" . htmlentities($file, ENT_QUOTES, "UTF-8") . "</a></li>";
                    }
                }
                // All files
                if( isset( $_GET["showFile"] ) ) {
                    foreach( $files as $file ) {
                        ...
                        if ( !in_array( strtolower($ext), $ext_allow ) ) continue;
